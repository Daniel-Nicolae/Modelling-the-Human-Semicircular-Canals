...
def compute_segments_dict(save_pickle=False):
    ...
    for key in col_dict.keys():
        segments_dict[key] = []
    for i, colour in enumerate(np.array(mesh.vertex_colors)):
        for key in segments_dict.keys():
            if np.linalg.norm(colour - np.array(col_dict[key])) < 1e-5:
                segments_dict[key].append(i)
    if save_pickle:
        with open('segments_dict.pickle', 'wb') as handle:
...
def get_canal_vertices(subject, canal):
    ...
    if canal == "anterior":
        vertex_indices = segments_dict["anterior"] + segments_dict["anterior-posterior"]
    elif canal == "posterior":
        vertex_indices = segments_dict["posterior"] + segments_dict["anterior-posterior"]
    elif canal == "lateral":
        vertex_indices = segments_dict["lateral"]
    else:
        raise ValueError("The canal can be either anterior, posterior or lateral. Canal "+canal+" is not a valid canal!")
    
    return np.array(mesh.vertices)[vertex_indices]
